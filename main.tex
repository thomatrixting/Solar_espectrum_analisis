...
\end{equation}
donde las constantes $h$, $c$ y $k_B$ corresponden a los valores CODATA 2014 \cite{mohrCODATARecommendedValues2015} provistos por la librería \texttt{astropy}. Ajustar $B_\lambda(T)$ al espectro observado $I_\lambda$ permite obtener la \textbf{temperatura de color} $T_\mathrm{color}$ del Sol.
...
\end{equation}
donde $\sigma = 5.6704\times10^{-8}\ \mathrm{W\,m^{-2}\,K^{-4}}$.
...
A partir de datos de Stellarium para el 27 de octubre de 2006 se obtuvieron los siguientes parámetros solares:
...
Distancia Tierra--Sol & $148\,672\,000\ \mathrm{km}$ \\
...
La inspección cuidadosa revela que curvas por encima de 5900 K se acercan más al máximo observado como se ve en (B), pero no intersectan ningún punto experimental previo al máximo, incumpliendo el segundo criterio. La temperatura que mas se aproxima al maximo y intersecta un punto experimental antes del máximo es:
\begin{equation*}
    T_\mathrm{color} = 5875\ \mathrm{K}.
\end{equation*}
El panel (D) muestra que este ajuste reproduce razonablemente bien el espectro observado; los residuales son atribuibles al ruido inherente a las mediciones del instrumento SIM.
...
\begin{equation*}
    F_\mathrm{comp} = 1337.9\ \mathrm{W\,m^{-2}}.
\end{equation*}
